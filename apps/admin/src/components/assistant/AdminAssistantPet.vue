    ...
    <transition name="assistant-panel-fade">
      <section v-if="panelVisible" class="assistant-panel">
        ...
        <div ref="messagesRef" class="assistant-messages">
          ...
          >
            <strong>{{ message.role === "assistant" ? "副驾" : "你" }}</strong>
            <p>{{ message.content }}</p>
          </article>
...
import { assistantApi } from "../../api/modules";
...
type AssistantSkin = "teal" | "cobalt" | "ember";
...
const compactScene = ref<AssistantScene>("none");
...
const messages = ref<UiMessage[]>([
  {
    ...
    role: "assistant",
    content:
      "我已经就位。默认不会自动读取后台当前页；你可以直接发需求，需要页内分析时再打开“已带页文”。",
  },
]);
...
const compactWorkMode = computed<AssistantWorkMode>(() => {
  if (currentAnimation.value === "sleep") {
    return "sleep";
  }
  ...
  if (
    loading.value ||
    speaking.value ||
    currentAnimation.value === "ponder"
  ) {
    return "coding";
  }

  if (panelVisible.value || includePageContext.value) {
    return "review";
  }
...
const panelWorkMode = computed<AssistantWorkMode>(() => {
  if (currentAnimation.value === "sleep") {
    return "sleep";
  }
...
const panelScene = computed<AssistantScene>(() => "none");

const petLabel = computed(() => {
  if (loading.value) return "分析中";
  return panelVisible.value ? "编辑副驾" : "副驾";
});

const panelSubtitle = computed(() => {
  const modeLabel = includePageContext.value ? "页内分析" : "自由对话";
  ...
  () => {
    ...
    showBubble(
      includePageContext.value
        ? `到了「${route.meta?.title || document.title || "当前页面"}」，现在会结合这页来分析。`
        : "后台页面切换了。我继续待命，不会自动读这页。",
...
watch(includePageContext, (enabled, previous) => {
  ...
  idleLines.value = getDefaultIdleLines();
  showBubble(enabled ? "已切到页内分析模式。" : "已切到自由对话模式。", {
    animation: "nod",
    duration: 3200,
  });
  scheduleAutonomy();
...
onMounted(() => {
  ...
  showBubble("轻点我试试。我会跟着你，也会自己提醒你下一步。", {
    animation: "wave",
    duration: 5200,
  });
  scheduleAutonomy();
...
function getLegacyDefaultPosition() {
  ...
  return {
    x: Math.max(16, window.innerWidth - 108),
...
function loadStoredPosition() {
  ...
  try {
    const raw = window.localStorage.getItem("aurora-admin-assistant-position");
    if (raw) {
...
function loadStoredSkin(): AssistantSkin {
  ...
  const raw = window.localStorage.getItem("aurora-admin-assistant-skin");
  if (raw === "teal" || raw === "cobalt" || raw === "ember") {
    return raw;
  }
...
function loadStoredPetKind(): AssistantPetKind {
  ...
  return window.localStorage.getItem("aurora-admin-assistant-pet") === "dog"
    ? "dog"
    : "cat";
}
...
function loadStoredAutonomyEnabled() {
  ...
  return (
    window.localStorage.getItem("aurora-admin-assistant-autonomy") !== "off"
  );
}

function saveSkin() {
  if (typeof window === "undefined") return;
  window.localStorage.setItem("aurora-admin-assistant-skin", skin.value);
}

function savePetKind() {
  if (typeof window === "undefined") return;
  window.localStorage.setItem("aurora-admin-assistant-pet", petKind.value);
}

function saveAutonomyEnabled() {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(
    "aurora-admin-assistant-autonomy",
...
function savePosition() {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(
    "aurora-admin-assistant-position",
...
function handlePointerUp() {
  ...
  if (!moved) {
    ...
    showBubble(
      panelVisible.value ? "我展开了，直接把需求丢给我。" : getIdleLine(),
      { animation: panelVisible.value ? "nod" : "wave", duration: 3600 },
    );
  }
}

function cycleSkin() {
  const skins: AssistantSkin[] = ["teal", "cobalt", "ember"];
...
function toggleAutonomy() {
  ...
  showBubble(autonomyEnabled.value ? "我会更主动提醒你。" : "我先安静待命。", {
    animation: "nod",
    duration: 2600,
  });
}
...
function buildDefaultActions(path: string): AssistantQuickAction[] {
  if (path.includes("/posts/new") || path.includes("/edit")) {
    return [
      {
        id: "title",
        label: "优化标题",
      ...
      },
      {
        id: "excerpt",
        label: "生成摘要",
        prompt: "请根据当前内容生成一段适合摘要栏的文案。",
      },
      {
        id: "seo",
        label: "SEO 检查",
        prompt: "请检查当前文章页面里的 SEO 缺口。",
      },
      {
        id: "tags",
        label: "标签建议",
        prompt: "请基于当前内容推荐更合适的标签。",
      },
    ];
  }
  ...
  if (path.includes("/comments")) {
    return [
      ...
      },
      {
        id: "risk",
        label: "风险排查",
        prompt: "请帮我判断当前评论是否有垃圾内容风险。",
      },
    ];
  }

  return [
    { id: "summary", label: "当前页总结", prompt: "请总结当前后台页面重点。" },
    {
      id: "next",
      label: "下一步建议",
...
function clearScene() {
  compactScene.value = "none";
  clearSceneTimer();
}

function triggerAutonomyScene() {
  const scenePool: Array<{
  ...
  const nextScene = scenePool[Math.floor(Math.random() * scenePool.length)];
  compactScene.value = nextScene.scene;
  currentMood.value = nextScene.mood;
  showBubble(nextScene.bubble, {
    animation: nextScene.animation,
...
function scheduleAutonomy() {
  ...
  autonomyTimer = window.setTimeout(() => {
    if (!loading.value) {
      const shouldPlayScene = !panelVisible.value && Math.random() > 0.48;
      if (shouldPlayScene) {
        triggerAutonomyScene();
      } else {
        clearScene();
...
function collectContext(): AssistantContext {
  ...
  const contentRoot = shouldIncludePageContext
  ...
  return {
    ...
    pageTitle: document.title || String(route.meta?.title || "Aurora Admin"),
    ...
    tags: Object.values(route.params).map((item) => String(item)),
    metadata: {
      includePageContext: shouldIncludePageContext,
      routeName: String(route.name ?? ""),
      routeTitle: String(route.meta?.title ?? ""),
      menuKey: String(route.meta?.menuKey ?? ""),
    },
  };
}
...
async function submitPrompt(input = prompt.value) {
  ...
  try {
    ...
    messages.value.push({
      ...
      content: formatAssistantError(
        error,
        "我暂时没有连上后台助手接口。先确认 API 正在运行，再继续问我。",
      ),
    });
...
.assistant-dock[data-skin="cobalt"] {
  --assistant-accent: #2b63ff;
  --assistant-accent-strong: #1746d3;
  --assistant-accent-soft: rgb(43 99 255 / 0.14);
  --assistant-accent-soft-strong: rgb(43 99 255 / 0.24);
  --assistant-panel-bg:
    radial-gradient(
      circle at top right,
      rgb(59 130 246 / 0.24),
  ...
  --assistant-pet-bg:
    ...
    linear-gradient(180deg, rgb(43 99 255 / 0.2), rgb(43 99 255 / 0.12));
  --assistant-face-bg: linear-gradient(
    180deg,
    rgb(242 246 251 / 0.98),
    rgb(205 219 235 / 0.95)
  );
  --assistant-pet-shadow: 0 18px 42px rgb(43 99 255 / 0.28);
}

.assistant-dock[data-skin="ember"] {
  --assistant-accent: #d55b12;
  --assistant-accent-strong: #ae430c;
  --assistant-accent-soft: rgb(213 91 18 / 0.14);
  --assistant-accent-soft-strong: rgb(213 91 18 / 0.24);
  --assistant-panel-bg:
    radial-gradient(
      circle at top right,
      rgb(249 115 22 / 0.24),
  ...
  --assistant-pet-bg:
    ...
    linear-gradient(180deg, rgb(213 91 18 / 0.2), rgb(213 91 18 / 0.12));
  --assistant-face-bg: linear-gradient(
    180deg,
    rgb(249 244 239 / 0.98),
    rgb(231 208 191 / 0.95)
  );
  --assistant-pet-shadow: 0 18px 42px rgb(213 91 18 / 0.28);
}
...
[data-admin-theme="dark"] .assistant-dock {
  --assistant-panel-bg:
    radial-gradient(
      circle at top right,
  ...
  --assistant-pet-bg:
    radial-gradient(circle at top, rgb(80 92 110 / 0.96), rgb(60 71 88 / 0.96)),
    linear-gradient(
      180deg,
...
[data-admin-theme="dark"] .assistant-close {
  background: rgb(255 255 255 / 0.14);
}

[data-admin-theme="dark"] .assistant-input {
